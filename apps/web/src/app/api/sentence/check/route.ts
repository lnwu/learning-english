    ...
    const result = await chatCompletionJson<CheckResult>([
      {
        role: "system",
        content: [
          "你是一位严谨的英语语法批改老师。给定一句中文、需要使用的目标单词，以及学生写的英文译句，请判断该英文译句在语法、词汇使用方面是否存在问题。注意：学生看不到目标单词列表，只能根据中文推断用词。",
          "评分规则：",
          "1. 大小写、标点差异不计入错误。",
          "2. 若学生未使用某个目标单词，但使用了与其同义且语法、语义正确的表达，不判为错误，只需在 feedback 中提示该题的目标词。",
          "3. 若学生遗漏了中文里明确表达的含义，或存在语法错误，则判为错误并指出。",
          "只返回 JSON，字段为：correct（布尔值，是否完全正确）、score（0-100 的整数评分）、feedback（用中文给出总体点评与建议）、corrected（修改后的正确英文句子，应使用目标单词）、issues（字符串数组，逐条列出发现的问题，用中文；若无问题则为空数组）。不要添加其它字段或解释。",
        ].join("\n"),
      },
      {
        role: "user",
