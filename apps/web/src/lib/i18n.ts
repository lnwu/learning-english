...
const zh = {
    ...
    'profile.speedByLengthDesc': '拼写流畅度以同长度档其他单词耗时的中位数为参照（排除本词）；每档不足 5 条计时样本时回退按单词长度估算',
    ...
    'profile.normalizePartial': '已归一化 {renamed} 个单词，合并 {merged} 个；{failed} 个单词请求失败，可稍后重试',
...
const en: Record<TranslationKey, string> = {
    ...
    'profile.speedByLengthDesc': 'Fluency compares each word against the median typing time of other words in the same length group; falls back to a length estimate until a group has 5 samples',
    ...
    'profile.normalizePartial': 'Normalized {renamed} words and merged {merged}; {failed} words failed and can be retried later',
