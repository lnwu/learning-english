...
- `useFirestoreWords()` 只返回稳定 context；高频变化的 `syncing` 与 `pendingCount` 只通过 `useSyncStatus()` 暴露。派生数据使用 `Words` computed getter；新增写入口必须同步失效 `#masteryCache`。
- `mergeSnapshotIntoStore` 必须保持增量合并及现有支配判定，不得改成全量替换 `wordData`；`attemptHistory` 保留最近 30 条，老数据由 `parseWordDoc` 兜底，正确率回退全量统计。
