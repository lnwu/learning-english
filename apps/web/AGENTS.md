...
- 造句抽词策略（练习次数达标的词按 `calculatePriority` 加权抽取、少练的补位、数量在 2-3 之间随机）在 `lib/sentenceWords.ts`（纯函数 + 测试）；单词数下限统一用 `MIN_SENTENCE_WORDS`，不足时 hook 置 `insufficientWords` 布尔状态（不要再用字符串哨兵），错误文案走 `tNow`。
