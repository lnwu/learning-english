...
- 规则只做鉴权（本人读写、preview 匿名可写），**不做字段校验**：2026-09 曾尝试加入字段校验，规则内容被 Ruleset API 拒绝（400）且发布流程先销毁旧 ruleset、造成规则空窗事故（见 `docs/DEPLOYMENT.md`），已放弃该方案；字段合法性由客户端清洗保证，新增写入字段无需改规则。
- 规则全部是 `request.auth` 的 O(1) 检查，禁止引入 `get()`/`exists()`（规则求值读取会产生额外计费）。
